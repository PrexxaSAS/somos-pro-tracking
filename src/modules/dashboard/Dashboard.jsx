...
export function Dashboard({ pedidos, conductores, devoluciones = [], recogidas = [], pqrs = [], promesas = [], ciudades = [] }) {
 ...
 // Fecha lmite segn promesa: fecha_creacion + dias_plazo
 const fechaLimite = (p) => {
 ...
 // Pedidos vencidos: activos cuya fecha lmite ya pas
 ...
 // Pedidos en riesgo: activos que vencen maana o hoy
 ...
 // Tiempo medio de entrega (dias reales) 
 ...
 // Promedio de dias prometidos (de los destinos con promesa)
 ...
 const ESTADOS_ORDEN = [
  ...
  { key: "en_transito", label: "En Transito", color: "#7c3aed" },
  { key: "paqueteria", label: "Paqueteria", color: "#0891b2" },
 ...
 const stats = [
  ...
  { l: "Devoluciones",  v: devoluciones.length, c: "#dc2626", i: "" },
 ];
 return (
  <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
    ...
    <div style={{ background: "#fef2f2", border: "2px solid #fca5a5", borderRadius: 12,
     ...
     <div style={{ flex: 1 }}>
      ...
      <div style={{ fontSize: 13, color: "#991b1b", marginTop: 2 }}>
       ...
       {pqrsAbiertas.length > 3 && ` y ${pqrsAbiertas.length - 3} ms`}
      </div>
     </div>
    ...
    <div style={{ background: "#fffbeb", border: "2px solid #fcd34d", borderRadius: 12,
     ...
     <div style={{ flex: 1 }}>
      <div style={{ fontWeight: 800, color: "#d97706", fontSize: 15 }}>
       {enRiesgo.length} pedido(s) en riesgo de vencer hoy o maana
      </div>
   ...
   {/* Fila: Mtricas de tiempo + Cumplimiento */}
   <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
      ...
      <>
       ...
       <svg viewBox="0 0 120 70" style={{ width: 120, height: 70, margin: "0 auto", display: "block" }}>
        ...
        <path d="M10,60 A50,50 0 0,1 110,60" fill="none"
         ...
         strokeDiasharray={`${pctCumpl * 1.57} 157`}/>
    ...
    <Card style={{ padding: 22 }}>
     ...
     {[
      ...
      { label: "En Gestion", color: "#d97706", bg: "#fffbeb", val: pqrs.filter(p=>p.estado==="en_gestion").length },
      { label: "Cerradias",  color: "#059669", bg: "#ecfdf5", val: pqrs.filter(p=>p.estado==="cerrada").length },
     ].map(row => (
   ...
   {/* Distribucin de estados (barras horizontales) */}
   <Card>
