...
function ModalDetalle({ pedido, conductores, ciudades, onClose, setPedidos, showToast, canEdit }) {
  ...
  const guardar = async () => {
    ...
    if (error) { showToast("Error guardando: "+error.message+" ("+error.code+")","error"); return; }
        ...
        <Modal title="Nueva Solicitud de Devolución" onClose={()=>{setModNueva(false);setForm(vacio);}} wide>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            {(form.tipo_envio||"conductor")==="empresa_transporte"&&(
              <Field label="Nombre Empresa" value={form.paqueteria||""} onChange={f("paqueteria")} placeholder="Logística Colombia S.A.S"/>
            )}
