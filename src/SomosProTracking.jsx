...
function MisPedidosConductor({ pedidos, user, conductores, ciudades, showToast, recargar }) {
  ...
  const marcarEntregado = async (pedido, fotos, conNovedad) => {
    ...
    const cambios = {
      ...
      soportes_data: [...(pedido.soportes_data||[]),...fotos],
      estado: estadoFinal,
      fecha_real: hoy,
      novedad: conNovedad,
    };
    ...
    showToast(`✓ Pedido marcado como ${estadoFinal==="entregado"?"Entregado":"Con Novedad"} · ${fotos.length} soporte(s) guardados`, "success");
    setModFotos(null);
    if (recargar) await recargar();
  };
              ...
              {modMapa?.id===p.id&&(
                <div style={{marginTop:14,borderRadius:12,overflow:"hidden",border:`2px solid ${P[200]}`}}>
                  <iframe title={"mapa-"+p.id} width="100%" height="280" style={{border:"none",display:"block"}}
                    src={`https://maps.google.com/maps?q=${encodeURIComponent((p.direccion||"")+", "+(ciudad?.name||p.ciudad_nombre||"")+", Colombia")}&output=embed&z=15`}
                    allowFullScreen loading="lazy"/>
                  <div style={{background:P[50],padding:"8px 14px",fontSize:12,color:P[700]}}>
                    📍 {p.direccion}, {ciudad?.name}
