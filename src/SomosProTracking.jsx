  ...
  const aplicar = async () => {
    ...
    for (const m of paraActualizar) {
      const cambios = {
        ...
        tipo:            "paqueteria",           // siempre paquetería cuando viene del CSV
        paqueteria:      m.paqueteria || null,
        fecha_estimada:  m.pedido.fecha_estimada || fechaEstStr, // respeta si ya tiene
      };
      if (m.destino) {
...
function Ciudades({ ciudades, showToast, recargar }) {
  ...
  const guardar=async()=>{
    ...
    if(supabase){
      const {error} = await supabase.from('ciudades').upsert({code:form.code.trim(),name:form.name.trim()},{onConflict:'code'});
    ...
    <div>
      ...
      {modCSV&&(
        <ModalCSVCiudades onClose={()=>setModCSV(false)} onImportar={async (nuevas)=>{
          // Batch upsert in chunks of 100 to avoid Supabase limits
          const CHUNK = 100;
          let ok = 0; let errores = 0;
          for(let i=0; i<nuevas.length; i+=CHUNK){
            const chunk = nuevas.slice(i, i+CHUNK).map(c=>({code:c.code.trim(), name:c.name.trim()}));
            const { error } = await supabase.from('ciudades').upsert(chunk, {onConflict:'code'});
            if(error){ errores += chunk.length; console.error('Chunk error:', error.message); }
            else ok += chunk.length;
          }
          setModCSV(false);
          if(errores>0) showToast(`✓ ${ok} guardadas · ✗ ${errores} con error`,"warning");
          else showToast(`✓ ${ok} ciudad(es) importadas correctamente`,"success");
          if(recargar) await recargar();
        }} />
      )}
    </div>
...
function SidebarApp({ user, activeTab, setActiveTab, onLogout, collapsed, setCollapsed, pqrs = [] }) {
  const MENUS = {
    admin:        [["dashboard","📊","Dashboard"],["pedidos","📦","Pedidos"],["rastreo","🗺️","Rastreo GPS"],["conductores","🚗","Conductores"],["transportistas","🏢","Transportistas"],["resumen","📋","Resumen Transportador"],["devoluciones","↩️","Devoluciones"],["recogidas","🔄","Recogidas"],["pqrs","📋","PQRS"],["ciudades","🏙️","Ciudades / DANE"],["paqueterias","📦","Paqueterías"],["promesas","📅","Promesas de Servicio"],["usuarios","👥","Usuarios"]],
  ...
  const cargarTodo = async () => {
    ...
    try {
      const [
        ...
        supabase.from('ciudades').select('*').order('name').range(0, 9999),
