...
function Transportistas({ transportistas, setTransportistas, conductores, setConductores, showToast, user, usuarios, setUsuarios }) {
  ...
  const [modEdit,     setModEdit]     = useState(null);   // conductor a editar
  ...
  const guardarEdicionConductor = () => {
    if (!formEdit.nombre.trim()||!formEdit.placa.trim()) { showToast("Nombre y placa son obligatorios","error"); return; }
    setConductores(prev=>prev.map(c=>c.id===modEdit.id?{...c,...formEdit}:c));
    // Update linked user too
    setUsuarios(prev=>prev.map(u=>u.id===modEdit.id?{...u,nombre:formEdit.nombre,placa:formEdit.placa,celular:formEdit.celular,nit_proveedor:formEdit.nit_proveedor,empresa:formEdit.empresa}:u));
    setModEdit(null);
    ...
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))", gap:14 }}>
        {misCon.map(c=>(
          <Card key={c.id} style={{ borderLeft:`3px solid ${P[400]}` }}>
            ...
            {c.nit_proveedor&&<div style={{fontSize:11,color:"#94a3b8"}}>NIT: {c.nit_proveedor}</div>}
      ...
      {modEdit && (
        <Modal title={`Editar Conductor — ${modEdit.nombre}`} onClose={()=>setModEdit(null)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <Field label="Nombre completo" value={formEdit.nombre}   onChange={v=>setFormEdit(p=>({...p,nombre:v}))}   required/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
            ...
            <Field label="Empresa" value={formEdit.empresa} onChange={v=>setFormEdit(p=>({...p,empresa:v}))} placeholder="Transportes XYZ S.A.S"/>
            <div style={{background:"#fffbeb",borderRadius:8,padding:"8px 12px",fontSize:12,color:"#92400e"}}>
              💡 El usuario y contraseña del conductor se modifican desde el módulo Usuarios.
            </div>
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              <Btn variant="secondary" onClick={()=>setModEdit(null)}>Cancelar</Btn>
              <Btn onClick={guardarEdicionConductor}>💾 Guardar Cambios</Btn>
            </div>
          </div>
        </Modal>
      )}
...
function Usuarios({ usuarios, setUsuarios, showToast }) {
  ...
  const abrirEditar = (u) => {
    setForm({ nombre:u.nombre, user:u.user, pass:"", rol:u.rol,
              nit:u.nit||"", empresa:u.empresa||"",
              placa:u.placa||"", nit_proveedor:u.nit_proveedor||"", celular:u.celular||"" });
    setModEditar(u);
  };

  const crearUsuario = () => {
    if (!form.nombre.trim()||!form.user.trim()||!form.pass.trim()) {
      showToast("Nombre, usuario y contraseña son obligatorios","error"); return;
    }
    if (usuarios.find(u=>u.user===form.user.trim())) {
  ...
  const guardarEdicion = () => {
    if (!form.nombre.trim()||!form.user.trim()) {
      showToast("Nombre y usuario son obligatorios","error"); return;
    }
    if (usuarios.find(u=>u.user===form.user.trim()&&u.id!==modEditar.id)) {
      showToast("Ese usuario ya lo usa otra persona","error"); return;
    }
    const passNueva = form.pass.trim() || modEditar.pass;
    setUsuarios(prev=>prev.map(u=>u.id===modEditar.id?{...u,...form,pass:passNueva}:u));
    setModEditar(null);
    showToast(form.pass.trim()?"✓ Usuario y contraseña actualizados":"✓ Usuario actualizado (contraseña sin cambios)","success");
  };

  const eliminar = (uid) => {
    if (uid===1) { showToast("No se puede eliminar el administrador principal","error"); return; }
    if (!window.confirm("¿Eliminar este usuario?")) return;
    setUsuarios(prev=>prev.filter(u=>u.id!==uid));
    showToast("Usuario eliminado","info");
  };

  // Campos extra según rol — inline, no sub-componente
  const camposRol = () => {
          ...
          <Card key={u.id} style={{borderTop:`3px solid ${roleColors[u.rol]||P[400]}`}}>
            <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:12}}>
              <div style={{width:40,height:40,borderRadius:20,
                background:`linear-gradient(135deg,${roleColors[u.rol]},${roleColors[u.rol]}99)`,
            ...
            <span style={{background:`${roleColors[u.rol]}18`,color:roleColors[u.rol],
      ...
      {modal && (
        <Modal title="Nuevo Usuario" onClose={()=>setModal(false)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              <Btn variant="secondary" onClick={()=>setModal(false)}>Cancelar</Btn>
              <Btn onClick={crearUsuario}>💾 Crear Usuario</Btn>
            </div>
          </div>
        </Modal>
      )}
...
function ModuloPQRS({ pqrs, setPqrs, pedidos, showToast, user }) {
  const MOTIVOS = [
  ...
  const guardarGestion = () => {
    ...
    setPqrs(prev=>prev.map(p=>p.id===modGestion.id?{
      ...p, respuesta:gestion, gestionado_por:user.nombre||user.user,
...
export default function SomosProTracking() {
  ...
  const renderContent = () => {
    switch (tab) {
      ...
      case "pedidos":        return <Pedidos pedidos={pedidos} setPedidos={sbSetPedidos} conductores={conductores} ciudades={ciudades} showToast={showToast} paqueterias={paqueterias}/>;
      case "rastreo":        return <RastreoGPS pedidos={pedidos} conductores={conductores} ciudades={ciudades}/>;
      case "conductores":    return <Conductores conductores={conductores} setConductores={sbSetConductores} pedidos={pedidos} showToast={showToast} usuarios={usuarios} setUsuarios={sbSetUsuarios} transportistas={transportistas}/>;
      case "transportistas": return <Transportistas transportistas={transportistas} setTransportistas={sbSetTransportistas} conductores={conductores} setConductores={sbSetConductores} showToast={showToast} user={{rol:"admin",nombre:"Admin"}} usuarios={usuarios} setUsuarios={sbSetUsuarios}/>;
      ...
      case "ciudades":       return <Ciudades ciudades={ciudades} setCiudades={sbSetCiudades} showToast={showToast}/>;
      case "paqueterias":    return <GestionPaqueterias paqueterias={paqueterias} setPaqueterias={sbSetPaqueterias} showToast={showToast}/>;
      case "usuarios":       return <Usuarios usuarios={usuarios} setUsuarios={sbSetUsuarios} showToast={showToast}/>;
      case "mi_empresa":     return <Transportistas transportistas={transportistas} setTransportistas={sbSetTransportistas} conductores={conductores} setConductores={sbSetConductores} showToast={showToast} user={user} usuarios={usuarios} setUsuarios={sbSetUsuarios}/>;
      case "mis_pedidos":    return <MisPedidosConductor pedidos={pedidos} setPedidos={sbSetPedidos} user={user} conductores={conductores} ciudades={ciudades} showToast={showToast}/>;
      ...
      case "pqrs":           return <ModuloPQRS pqrs={pqrs} setPqrs={sbSetPqrs} pedidos={pedidos} showToast={showToast} user={user}/>;
      case "devoluciones":   return <ModuloDevoluciones devoluciones={devoluciones} setDevoluciones={sbSetDevoluciones} conductores={conductores} ciudades={ciudades} showToast={showToast} user={user}/>;
      case "recogidas":      return <ModuloRecogidas recogidas={recogidas} setRecogidas={sbSetRecogidas} conductores={conductores} ciudades={ciudades} showToast={showToast} user={user}/>;
