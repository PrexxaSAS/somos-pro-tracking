...
function ModalDetalle({ pedido, conductores, ciudades, onClose, setPedidos, showToast, canEdit }) {
  ...
  const guardar = async () => {
    ...
    const cambios = {
      ...
      novedad,
      factura: facturaEdit || pedido.factura,
      fecha_estimada: fechaEdit || pedido.fecha_estimada,
      tipo: tipoModal,
        ...
        {canEdit&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            {tipoModal==="empresa_transporte"&&(
              <Field label="Empresa de Transporte" value={empTrans} onChange={setEmpTrans} placeholder="Nombre empresa"/>
            )}
        ...
        <Modal title="Nuevo Pedido" onClose={() => setModNuevo(false)}>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {form.tipo === "empresa_transporte" && (
              <Field label="Nombre Empresa de Transporte" value={form.empresa_transporte} onChange={f("empresa_transporte")} placeholder="Logística Colombia S.A.S" />
            )}
...
function ModalCSVCiudades({ onClose, onImportar }) {
  ...
  const leerArchivo = (file) => {
    ...
    const reader = new FileReader();
    reader.onload = (e) => {
      setTxt(e.target.result); setErr("");
      try { setPrev(parsear(e.target.result)); } catch(ex) { setErr(ex.message); setPrev([]); }
    };
    reader.readAsText(file, 'UTF-8');
  };
  ...
  const parsear = (texto) => {
    const lineas = texto.trim().split("\n").filter(l=>l.trim());
    if (lineas.length < 2) throw new Error("Se necesita encabezado code,name y al menos una fila.");
    const sep = lineas[0].includes(';') ? ';' : ',';
    const hdrs = lineas[0].split(sep).map(h=>h.trim().toLowerCase().replace(/"/g,''));
    return lineas.slice(1).map(l=>{
      ...
      return { code:obj.code.trim(), name:obj.name.trim() };
    }).filter(Boolean);
  };

  const importar = async () => {
    if (prev.length===0) { setErr("Carga un archivo primero."); return; }
    setCargando(true);
    try { await onImportar(prev); }
  ...
  return (
    <Modal title="Importar Ciudades / Códigos DANE" onClose={onClose} wide>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        <div style={{background:"#fffbeb",borderRadius:10,padding:12,fontSize:13,color:"#92400e"}}>
          <strong>Columnas requeridas:</strong> <code>code</code> (código DANE 5 dígitos) y <code>name</code> (nombre municipio)
        </div>
        <Btn size="sm" variant="success" onClick={()=>descargarCSV("plantilla_ciudades.csv",CABECERA,EJEMPLO)}>⬇ Descargar Plantilla</Btn>
        <div style={{border:`2px dashed ${P[300]}`,borderRadius:12,padding:"20px 16px",textAlign:"center",cursor:"pointer",background:archivo?"#f0fdf4":"#fafafa"}}
          onClick={()=>fileRef.current&&fileRef.current.click()}
          onDragOver={e=>e.preventDefault()}
          onDrop={e=>{e.preventDefault();leerArchivo(e.dataTransfer.files[0]);}}>
          <div style={{fontSize:28,marginBottom:6}}>📂</div>
          {archivo?<div style={{color:"#059669",fontWeight:700}}>✓ {archivo}</div>:<div style={{color:P[600],fontWeight:600}}>Clic para seleccionar archivo CSV</div>}
        </div>
        <input ref={fileRef} type="file" accept=".csv,.txt" style={{display:"none"}} onChange={e=>leerArchivo(e.target.files[0])}/>
        {err&&<p style={{color:"#dc2626",background:"#fef2f2",padding:"8px 12px",borderRadius:8,fontSize:13,margin:0}}>⚠️ {err}</p>}
        {prev.length>0&&(
          <div style={{background:"#f0fdf4",borderRadius:10,padding:12,border:"1px solid #86efac",maxHeight:160,overflowY:"auto"}}>
            <p style={{margin:"0 0 6px",fontWeight:700,color:"#15803d",fontSize:13}}>✓ {prev.length} ciudad(es) listas</p>
            {prev.slice(0,8).map((c,i)=><div key={i} style={{fontSize:12,color:"#334155"}}>• {c.code} — {c.name}</div>)}
            {prev.length>8&&<div style={{fontSize:11,color:"#94a3b8"}}>...y {prev.length-8} más</div>}
          </div>
        )}
        <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
          <Btn variant="secondary" onClick={onClose}>Cancelar</Btn>
          <Btn disabled={prev.length===0||cargando} onClick={importar}>{cargando?"⏳ Importando...":`📥 Importar (${prev.length})`}</Btn>
        </div>
      </div>
    </Modal>
  );
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
        {paqueterias.map((p,i)=>(
          <Card key={i} style={{padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <button onClick={async ()=>{const{error:paqErr}=await supabase.from('paqueterias').delete().eq('nombre',p); if(!paqErr){if(recargar)if(recargar) await recargar(); else if(window._recargar) await window._recargar();} showToast("Eliminado","info");}}
...
//  MÓDULO DEVOLUCIONES
// ══════════════════════════════════════════════════════════
function ModuloDevoluciones({ devoluciones, conductores, ciudades, showToast, user, recargar }) {
  const [modNueva, setModNueva] = useState(false);
  const [modDet,   setModDet]   = useState(null);
  const [busq,     setBusq]     = useState("");
  const fileRef = useRef(null);
  ...
  const asignar = async (id, condId, novedad) => {
    ...
    await supabase.from('devoluciones').update(cambios).eq('id',id); if(recargar) await recargar();
  };
      ...
      {modNueva && (
        <Modal title="Nueva Solicitud de Devolución" onClose={()=>{setModNueva(false);setForm(vacio);}} wide>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <Field label="Tipo de Envío Devolución" value={form.tipo_envio||"conductor"} onChange={f("tipo_envio")} as="select"
              options={[{value:"conductor",label:"🚗 Conductor Propio"},{value:"paqueteria",label:"📦 Paquetería Tercero"}]}/>
            {(form.tipo_envio||"conductor")==="paqueteria"?(
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                ...
                <Field label="N° Guía" value={form.guia_paqueteria||""} onChange={f("guia_paqueteria")} placeholder="SRV-2026-XXXX"/>
              </div>
            ):(
              <Field label="Asignar Conductor (opcional)" value={form.conductor_id} onChange={f("conductor_id")} as="select"
                options={[{value:"",label:"— Sin asignar —"},...conductores.map(c=>({value:c.id,label:`${c.nombre} · ${c.placa}`}))]}/>
        ...
        {dev.soporte_data&&(
          <div style={{background:"#f0fdf4",borderRadius:10,padding:12,border:"1px solid #86efac"}}>
            ...
            {dev.soporte_data.startsWith("data:image")?(
              ...
              <Btn size="sm" variant="success" onClick={()=>window.open(dev.soporte_data?.startsWith("data:")?dev.soporte_data:"data:application/octet-stream;base64,"+dev.soporte_data,"_blank")}>Ver PDF</Btn>
            )}
          </div>
        )}
...
//  MÓDULO RECOGIDAS
// ══════════════════════════════════════════════════════════
function ModuloRecogidas({ recogidas, conductores, ciudades, showToast, user, recargar }) {
  const [modNueva, setModNueva] = useState(false);
  const [modDet,   setModDet]   = useState(null);
  const [busq,     setBusq]     = useState("");
  const fileRef = useRef(null);
  ...
  const asignar = async (id, condId, novedad) => {
    ...
    await supabase.from('recogidas').update(cambios).eq('id',id); if(recargar) await recargar();
  };
      ...
      {modNueva && (
        <Modal title="Nueva Solicitud de Recogida" onClose={()=>{setModNueva(false);setForm(vacio);}} wide>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <Field label="Tipo de Envío Recogida" value={form.tipo_envio||"conductor"} onChange={f("tipo_envio")} as="select"
              options={[{value:"conductor",label:"🚗 Conductor Propio"},{value:"paqueteria",label:"📦 Paquetería Tercero"}]}/>
            {(form.tipo_envio||"conductor")==="paqueteria"?(
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                <Field label="Empresa Paquetería" value={form.paqueteria||""} onChange={f("paqueteria")} placeholder="Servientrega, TCC..."/>
        ...
        {rec.doc_data&&(
          <div style={{background:"#f0fdf4",borderRadius:10,padding:12,border:"1px solid #86efac"}}>
            ...
            {rec.doc_data.startsWith("data:image")?(
              ...
              <Btn size="sm" variant="success" onClick={()=>window.open(rec.doc_data?.startsWith("data:")?rec.doc_data:"data:application/octet-stream;base64,"+rec.doc_data,"_blank")}>Ver PDF</Btn>
            )}
          </div>
        )}
        {canEdit&&(
...
function MisPedidosConductor({ pedidos, user, conductores, ciudades, showToast, recargar }) {
  ...
  const misPeds = pedidos.filter(p => p.conductor_id && String(p.conductor_id) === String(condId));
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))", gap:14 }}>
        {misPeds.map(p=>(
          <Card key={p.id} style={{ cursor:"pointer", borderLeft:`4px solid ${ESTADOS_PEDIDO[p.estado]?.color||"#ccc"}` }}
            onClick={()=>setModDet(p)}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 }}>
              <div>
                <div style={{ fontWeight:900, color:P[600], fontSize:18, fontFamily:"monospace" }}>
                  {p.guia_interna||p.id}
                </div>
                <div style={{ fontSize:12, color:"#94a3b8", marginTop:2 }}>
                  Pedido: <strong style={{color:P[700]}}>{p.id}</strong> · Factura: <strong style={{color:"#334155"}}>{p.factura}</strong>
                </div>
              </div>
              <Badge estado={p.estado}/>
            </div>
            ...
            <div style={{ fontSize:13, color:"#64748b", display:"flex", flexDirection:"column", gap:3 }}>
              ...
              <span>🏙️ {p.ciudad_nombre} · 📍 {p.direccion}</span>
              {p.fecha_estimada&&<span>📅 Estimado: {p.fecha_estimada}</span>}
            </div>
          </Card>
        ))}
      </div>
      {modDet&&<ModalDetalle pedido={modDet} conductores={conductores} ciudades={ciudades}
          ...
          <Card key={p.id} style={{cursor:"pointer",borderLeft:`4px solid ${ESTADOS_PEDIDO[p.estado]?.color||"#ccc"}`}} onClick={()=>setModDet(p)}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
              <div>
                ...
                <div style={{fontSize:12,color:"#94a3b8"}}>Pedido: {p.id} · Factura: {p.factura}</div>
              </div>
              <Badge estado={p.estado}/>
            </div>
            <div style={{fontWeight:700,color:"#1e293b",marginBottom:6}}>{p.cliente}</div>
...
export default function SomosProTracking() {
  ...
  const renderContent = () => {
    ...
    switch (tab) {
      ...
      case "pedidos":        return <Pedidos pedidos={pedidos} setPedidos={sbSetPedidos} conductores={conductores} ciudades={ciudades} showToast={showToast} paqueterias={paqueterias} recargar={cargarTodo}/>;
      ...
      case "devoluciones":   return <ModuloDevoluciones devoluciones={devoluciones} conductores={conductores} ciudades={ciudades} showToast={showToast} user={user} recargar={cargarTodo}/>;
      case "recogidas":      return <ModuloRecogidas recogidas={recogidas} conductores={conductores} ciudades={ciudades} showToast={showToast} user={user} recargar={cargarTodo}/>;
