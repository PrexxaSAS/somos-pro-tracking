...
// ─── Generador de guía interna única ─────────────────────────────────────────
function generarGuia(pedidos) {
  const year = new Date().getFullYear();
  const usados = pedidos
    .map(p => p.guia_interna)
    .filter(g => g && g.startsWith(`SPT-${year}-`))
    .map(g => parseInt(g.split("-")[2]) || 0);
  const max = usados.length ? Math.max(...usados) : 0;
  return `SPT-${year}-${String(max + 1).padStart(4, "0")}`;
}

// ─── Descarga de CSV plantilla ────────────────────────────────────────────────
...
function GuiaImprimible({ pedido, conductores, ciudades, onClose }) {
  ...
  const esPropio = pedido.tipo === "propio" || pedido.tipo === "empresa_transporte" || pedido.tipo === "mensajeria";
  return (
    <Modal title={`Guía — ${pedido.guia_interna || pedido.id}`} onClose={onClose} wide>
      ...
      <div id="guia-print" style={{border:`2px solid ${P[200]}`,borderRadius:12,padding:28}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",paddingBottom:16,marginBottom:20,borderBottom:`3px solid ${P[600]}`}}>
          ...
          <div style={{textAlign:"right"}}>
            {pedido.guia_interna && (
              <div style={{background:P[600],color:"#fff",borderRadius:10,padding:"6px 16px",fontSize:18,fontWeight:900,marginBottom:4}}>
                {pedido.guia_interna}
              </div>
            )}
        ...
        <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}>
          ...
          <span style={{background:P[50],color:P[700],border:`1px solid ${P[200]}`,borderRadius:8,padding:"4px 12px",fontWeight:700,fontSize:12}}>
            {pedido.tipo==="paqueteria"?"📦 Paquetería":pedido.tipo==="empresa_transporte"?"🏢 Empresa Transporte":pedido.tipo==="mensajeria"?"🛵 Mensajería":"🚚 Propio"}
          </span>
        </div>
          ...
          <div style={{background:"#f8fafc",borderRadius:12,padding:16}}>
            ...
            {pedido.tipo==="paqueteria"?(
              ...
              <table style={{width:"100%",fontSize:14,borderCollapse:"collapse"}}>
                <tbody>
                  {[
                    ["Conductor:",cond?.nombre||"Por asignar"],
                    ["Placa:",pedido.placa||"—"],
                    ["Empresa:",cond?.empresa||pedido.empresa_transporte||"—"],
        ...
        <div style={{background:"#f8fafc",borderRadius:10,padding:14,marginBottom:20,display:"flex",gap:32,fontSize:13,flexWrap:"wrap"}}>
          ...
          {pedido.guia_interna&&<span><strong>Guía interna:</strong> {pedido.guia_interna}</span>}
        </div>
        ...
        <div style={{marginTop:20,paddingTop:14,borderTop:"1px dashed #cbd5e1",display:"flex",justifyContent:"space-between"}}>
          <div style={{fontSize:10,color:"#94a3b8"}}>Somos PRO Tracking · {fecha}</div>
...
function ModalDetalle({ pedido, conductores, ciudades, onClose, setPedidos, showToast, canEdit }) {
  ...
  const guardar = () => {
    ...
    setPedidos(prev=>prev.map(p=>p.id===pedido.id?{
      ...p,estado,
      conductor_id:c?.id||null,placa:c?.placa||null,
      nit_proveedor:c?.nit_proveedor||null,fecha_real:fechaReal||null,
    }:p));
      ...
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        <div style={{background:P[50],borderRadius:12,padding:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))",gap:8,fontSize:13,color:"#64748b"}}>
            ...
            {pedido.guia_interna&&<span>🔖 Guía: <strong style={{fontFamily:"monospace",color:P[600]}}>{pedido.guia_interna}</strong></span>}
          </div>
        ...
        {!canEdit&&cond&&(
          <div style={{background:"#eff6ff",borderRadius:10,padding:12}}>
            <span style={{fontSize:13,color:"#1e40af"}}>🚗 <strong>{cond.nombre}</strong> — Placa: <strong>{pedido.placa}</strong>{cond.celular&&` · 📱 ${cond.celular}`}</span>
          </div>
        )}
        ...
        <div>
          ...
          <div style={{display:"flex",gap:8}}>
            <input value={soporte} onChange={e=>setSoporte(e.target.value)}
              ...
              style={{...iSt,flex:1}} onKeyDown={e=>e.key==="Enter"&&subirSoporte()}/>
...
function ModalCSVPedidos({ onClose, onImportar, ciudades }) {
  ...
  const parsear=(texto)=>{
    ...
    return lineas.slice(1).map((l,idx)=>{
      ...
      const obj={};hdrs.forEach((h,i)=>{obj[h]=cols[i]||"";});
      const ciudad=ciudades.find(c=>c.code===obj.ciudad_codigo);
      const esPaq=obj.tipo==="paqueteria";
      return {
        id:obj.id||`IMP-${Date.now()}-${idx}`,guia_interna:null,
        cliente:obj.cliente||"Sin nombre",
        ciudad_codigo:ciudad?.code||obj.ciudad_codigo||"",ciudad_nombre:ciudad?.name||"",
        direccion:obj.direccion||"",cajas:parseInt(obj.cajas)||0,factura:obj.factura||"",
        fecha_estimada:obj.fecha_estimada||"",notas:obj.notas||"",
        ...
        paqueteria:esPaq?obj.paqueteria:null,guia_paqueteria:esPaq?obj.guia_paqueteria:null,
        conductor_id:null,placa:null,nit_proveedor:null,
        estado:esPaq?"paqueteria":"sin_asignar",
        fecha_creacion:new Date().toISOString().split("T")[0],fecha_real:null,soportes:[],
      };
    });
  };

  const cabecera="id,cliente,ciudad_codigo,direccion,cajas,factura,fecha_estimada,tipo,empresa_transporte,paqueteria,guia_paqueteria,notas";
  const ejemplo="PED-010,Empresa Ejemplo S.A.S,11001,Cra 10 #20-30 Of 201,5,FAC-3000,2026-05-10,propio,,,\nPED-011,Comercio del Norte Ltda,05001,Av El Poblado 43A-15,12,FAC-3001,2026-05-12,paqueteria,,Servientrega,SRV-2026-12345,\nPED-012,Mensajería Rápida,76001,Calle 5 #10-20,2,FAC-3002,2026-05-13,mensajeria,,,";
    ...
    <Modal title="Importar Pedidos desde CSV" onClose={onClose} wide>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        <div style={{background:"#fffbeb",borderRadius:10,padding:12,fontSize:13,color:"#92400e"}}>
          <strong>Columnas:</strong> id, cliente, ciudad_codigo, direccion, cajas, factura, fecha_estimada, tipo (propio/empresa_transporte/mensajeria/paqueteria), empresa_transporte, paqueteria, guia_paqueteria, notas
        </div>
        <Btn size="sm" variant="success"
          onClick={()=>descargarCSV("plantilla_pedidos.csv",cabecera,ejemplo)}>
          ⬇ Descargar Plantilla CSV
        </Btn>
        <textarea value={txt} onChange={e=>setPrev2(e.target.value)} rows={5}
          style={{...iSt,fontFamily:"monospace",fontSize:12,resize:"vertical"}}
          placeholder="Pega el contenido CSV aquí o descarga la plantilla arriba..."/>
        {err&&<p style={{color:"#dc2626",background:"#fef2f2",padding:"8px 12px",borderRadius:8,fontSize:13,margin:0}}>⚠️ {err}</p>}
        <Btn variant="secondary" onClick={()=>{setErr("");try{setPrev(parsear(txt));}catch(e){setErr(e.message);setPrev([]);}}}>👁 Previsualizar</Btn>
        {prev.length>0&&(
...
function ModalCSVCiudades({ onClose, onImportar }) {
  ...
  const [prev,setPrev]=useState([]);
  const [err,setErr]=useState("");
  const cabecera="codigo_dane,nombre_ciudad";
  const ejemplo="05045,Apartadó\n05088,Bello\n05380,Itagüí\n05631,Sabaneta\n76109,Buenaventura";
  ...
  const parsear=(texto)=>{
    ...
    return lineas.slice(1).map(l=>{
      const cols=l.split(",").map(c=>c.trim());
      const obj={};hdrs.forEach((h,i)=>{obj[h]=cols[i]||"";});
      return {code:(obj.codigo_dane||obj.code||"").trim(),name:(obj.nombre_ciudad||obj.name||"").trim()};
    }).filter(c=>c.code&&c.name);
  };

  return (
    <Modal title="Importar Ciudades desde CSV" onClose={onClose} wide>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        <div style={{background:"#fffbeb",borderRadius:10,padding:12,fontSize:13,color:"#92400e"}}>
          <strong>Columnas:</strong> codigo_dane, nombre_ciudad
        </div>
        ...
        {prev.length>0&&(
          <div style={{background:"#f0fdf4",borderRadius:10,padding:14,border:"1px solid #86efac"}}>
            <p style={{margin:"0 0 8px",fontWeight:700,color:"#15803d",fontSize:13}}>✓ {prev.length} ciudad(es) listas:</p>
            {prev.map(c=><div key={c.code} style={{fontSize:12,color:"#334155"}}>• {c.name} — {c.code}</div>)}
          </div>
        )}
        <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
          <Btn variant="secondary" onClick={onClose}>Cancelar</Btn>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{background:P[50]}}>
              {["Guía Interna","N° Pedido","Factura","Cliente","Ciudad","Cajas","Estado","Conductor"].map(h=>(
              ...
              {pedidos.slice(0,8).map((p,i)=>{
                ...
                return (
                  <tr key={p.id} style={{borderTop:`1px solid ${P[100]}`,background:i%2?"#fafafa":"#fff"}}>
                    ...
                    <td style={{padding:"11px 14px",color:"#334155",maxWidth:140,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.cliente}</td>
...
// ─── Pedidos ──────────────────────────────────────────────────────────────────
function Pedidos({ pedidos, setPedidos, conductores, ciudades, showToast, paqueterias }) {
  ...
  const vacio={id:"",cliente:"",ciudad_codigo:"",direccion:"",cajas:"",factura:"",fecha_estimada:"",notas:"",conductor_id:"",tipo:"propio",empresa_transporte:"",paqueteria:"",guia_paqueteria:""};
  const [form,setForm]=useState(vacio);
  const f=k=>v=>setForm(p=>({...p,[k]:v}));

  const TIPOS_ENVIO=[
    {value:"propio",label:"🚚 Transporte Propio"},
    {value:"empresa_transporte",label:"🏢 Empresa de Transporte"},
  ...
  const filtrados=pedidos.filter(p=>{
    ...
    const okB=!busq||p.id.toLowerCase().includes(q)||p.cliente.toLowerCase().includes(q)||p.factura?.toLowerCase().includes(q)||(p.ciudad_nombre||"").toLowerCase().includes(q)||(p.guia_interna||"").toLowerCase().includes(q);
    return okF&&okB;
  });
  ...
  const guardar=()=>{
    ...
    if(pedidos.find(p=>p.id===form.id.trim())){showToast("Ya existe un pedido con ese número","error");return;}
    ...
    const necesitaGuia=!esPaq;
    const guia_interna=necesitaGuia?generarGuia(pedidos):null;
    setPedidos(prev=>[{
      ...
      conductor_id:cond?cond.id:null,placa:cond?cond.placa:null,
      nit_proveedor:cond?cond.nit_proveedor:null,
      estado:esPaq?"paqueteria":(cond?"pendiente":"sin_asignar"),
      fecha_creacion:new Date().toISOString().split("T")[0],fecha_real:null,soportes:[],
    },...prev]);
    setModNuevo(false);setForm(vacio);
    showToast(`✓ Pedido ${form.id} creado · Guía: ${guia_interna||"N/A"}`,"success");
  };

  const imprimirPlanilla=()=>{
    const win=window.open("","_blank");
    if(!win){showToast("Permite ventanas emergentes","error");return;}
    win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>Planilla</title>
    ...
    td{padding:10px 12px;border-bottom:1px solid #ede9fe;font-size:12px}</style></head>
    <body><h1>Planilla de Despachos — Somos PRO Tracking</h1>
    <p>Fecha: ${new Date().toLocaleDateString("es-CO",{day:"2-digit",month:"long",year:"numeric"})} · Pedidos: ${filtrados.length} · Cajas: ${filtrados.reduce((a,p)=>a+(parseInt(p.cajas)||0),0)}</p>
    <table><thead><tr><th>#</th><th>Guía Interna</th><th>N° Pedido</th><th>Factura</th><th>Cliente</th><th>Ciudad</th><th>Dirección</th><th>Cajas</th><th>Estado</th><th>Conductor / Paquetería</th><th>Firma Recibido</th></tr></thead>
    <tbody>${filtrados.map((p,i)=>{
      const cond=conductores.find(c=>c.id===p.conductor_id);
      const trans=p.tipo==="paqueteria"?`📦 ${p.paqueteria} — ${p.guia_paqueteria}`:(cond?`${cond.nombre} · ${p.placa}`:"Sin asignar");
      return `<tr><td>${i+1}</td><td><strong>${p.guia_interna||"—"}</strong></td><td>${p.id}</td><td>${p.factura||"—"}</td><td>${p.cliente}</td><td>${p.ciudad_nombre}</td><td>${p.direccion}</td><td style="text-align:center"><strong>${p.cajas}</strong></td><td>${p.estado}</td><td>${trans}</td><td></td></tr>`;
    ...
    <div>
      ...
      <Card style={{padding:14,marginBottom:16}}>
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          <input value={busq} onChange={e=>setBusq(e.target.value)}
            placeholder="🔍 Buscar por N° pedido, guía, factura, cliente o ciudad..."
      ...
      <Card style={{padding:0,overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{background:P[50]}}>
              {["Guía Interna","N° Pedido","Factura","Cliente","Ciudad / DANE","Cajas","Estado","Conductor / Paquetería","Acciones"].map(h=>(
                <th key={h} style={{padding:"11px 14px",textAlign:"left",fontWeight:700,color:P[700],fontSize:11,whiteSpace:"nowrap"}}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {filtrados.length===0&&<tr><td colSpan={9} style={{padding:40,textAlign:"center",color:"#94a3b8"}}>Sin pedidos</td></tr>}
              {filtrados.map((p,i)=>{
                const cond=conductores.find(c=>c.id===p.conductor_id);
                return (
                  <tr key={p.id} style={{borderTop:`1px solid ${P[100]}`,background:i%2?"#fafafa":"#fff"}}>
      ...
      {modNuevo&&(
        <Modal title="Nuevo Pedido" onClose={()=>setModNuevo(false)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <Field label="Tipo de Envío" value={form.tipo} onChange={f("tipo")} as="select" options={TIPOS_ENVIO}/>
            {form.tipo==="paqueteria"?(
              ...
              <>
                {form.tipo==="empresa_transporte"&&(
                  <Field label="Nombre Empresa de Transporte" value={form.empresa_transporte} onChange={f("empresa_transporte")} placeholder="Transportes Rápidos S.A.S"/>
                )}
            ...
            <div style={{background:P[50],borderRadius:10,padding:10,fontSize:12,color:P[700]}}>
              🔖 Se generará automáticamente una <strong>Guía Interna</strong> (ej: SPT-2026-0006) para pedidos de transporte propio.
            </div>
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
      ...
      {modCSV&&<ModalCSVPedidos onClose={()=>setModCSV(false)} ciudades={ciudades} onImportar={rows=>{const conGuias=rows.map(r=>({...r,guia_interna:r.tipo!=="paqueteria"?generarGuia([...pedidos,...rows.slice(0,rows.indexOf(r))]):null}));setPedidos(p=>[...conGuias,...p]);setModCSV(false);showToast(`✓ ${rows.length} pedido(s) importados`,"success");}}/>}
    </div>
  );
}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,maxHeight:500,overflowY:"auto"}}>
            ...
            {conCond.map(p=>(
              <button key={p.id} onClick={()=>setSel(p)} style={{width:"100%",padding:"10px 12px",background:sel?.id===p.id?P[50]:"#fafafa",border:`1.5px solid ${sel?.id===p.id?P[400]:"#e2e8f0"}`,borderRadius:10,cursor:"pointer",textAlign:"left",fontFamily:"inherit"}}>
                <div style={{fontWeight:700,color:P[700],fontSize:11,fontFamily:"monospace"}}>{p.guia_interna||p.id}</div>
          ...
          {sel&&mapUrl?(
            <>
              <div style={{marginBottom:14}}>
                <div style={{fontWeight:800,color:P[800],fontSize:16}}>{sel.guia_interna||sel.id} — {sel.cliente}</div>
                <div style={{fontSize:13,color:"#64748b",marginTop:4}}>📍 {sel.direccion}, {ciudad?.name}</div>
                {cond&&<div style={{fontSize:13,color:P[600],marginTop:2}}>🚗 {cond.nombre} · Placa: {sel.placa}{cond.celular&&` · 📱 ${cond.celular}`}</div>}
              </div>
              <div style={{borderRadius:12,overflow:"hidden",border:`2px solid ${P[200]}`}}>
...
// ─── Conductores ──────────────────────────────────────────────────────────────
function Conductores({ conductores, setConductores, pedidos, showToast, usuarios, setUsuarios }) {
  const [modal,setModal]=useState(false);
  const [form,setForm]=useState({nombre:"",placa:"",celular:"",nit_proveedor:"",empresa:"",user:"",pass:""});
  const f=k=>v=>setForm(p=>({...p,[k]:v}));

  const guardar=()=>{
    if(!form.nombre.trim()||!form.placa.trim()){showToast("Nombre y placa son obligatorios","error");return;}
    if(!form.user.trim()||!form.pass.trim()){showToast("Usuario y contraseña son obligatorios","error");return;}
    if(usuarios.find(u=>u.user===form.user.trim())){showToast("Ese nombre de usuario ya existe","error");return;}
    const newId=Date.now();
    // Crea conductor
    setConductores(prev=>[...prev,{id:newId,nombre:form.nombre.trim(),placa:form.placa.trim(),celular:form.celular.trim(),nit_proveedor:form.nit_proveedor.trim(),empresa:form.empresa.trim(),activo:true}]);
    // Crea usuario vinculado con mismo ID
    setUsuarios(prev=>[...prev,{id:newId,nombre:form.nombre.trim(),user:form.user.trim(),pass:form.pass.trim(),rol:"conductor",placa:form.placa.trim(),celular:form.celular.trim(),nit_proveedor:form.nit_proveedor.trim(),empresa:form.empresa.trim()}]);
    setModal(false);
    setForm({nombre:"",placa:"",celular:"",nit_proveedor:"",empresa:"",user:"",pass:""});
    showToast(`✓ Conductor registrado · Usuario: ${form.user}`,"success");
  };
    ...
    <div>
      ...
      {modal&&(
        <Modal title="Registrar Conductor" onClose={()=>setModal(false)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{background:P[50],borderRadius:10,padding:10,fontSize:12,color:P[700]}}>
              Al registrar el conductor se crea automáticamente su usuario de acceso al sistema.
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              ...
              <Field label="Empresa" value={form.empresa} onChange={f("empresa")} placeholder="Transportes XYZ S.A.S"/>
            </div>
            <div style={{borderTop:`1px solid ${P[100]}`,paddingTop:12,marginTop:2}}>
              <p style={{fontSize:12,fontWeight:700,color:P[700],margin:"0 0 10px",textTransform:"uppercase"}}>Acceso al Sistema</p>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                <Field label="Usuario (login)" value={form.user} onChange={f("user")} required placeholder="juan.perez"/>
                <Field label="Contraseña" value={form.pass} onChange={f("pass")} required type="password" placeholder="••••••••"/>
              </div>
            </div>
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              <Btn variant="secondary" onClick={()=>setModal(false)}>Cancelar</Btn>
              <Btn onClick={guardar}>💾 Guardar y Crear Usuario</Btn>
            </div>
          </div>
        </Modal>
      )}
...
function Transportistas({ transportistas, setTransportistas, conductores, setConductores, showToast, user, usuarios, setUsuarios }) {
  ...
  const [modCond,setModCond]=useState(null); // empresa seleccionada para inscribir conductor
  const [formE,setFormE]=useState({nombre:"",nit:"",contacto:"",tel:""});
  const [formC,setFormC]=useState({nombre:"",placa:"",celular:"",user:"",pass:""});
  const fe=k=>v=>setFormE(p=>({...p,[k]:v}));
  const fc=k=>v=>setFormC(p=>({...p,[k]:v}));

  const esMia=user.rol==="transportista";
  const miNit=user.nit||"";
  const misEmpresas=esMia?transportistas.filter(t=>t.nit===miNit):transportistas;
  const misConductores=esMia?conductores.filter(c=>c.nit_proveedor===miNit):conductores;
  ...
  const crearEmpresa=()=>{
    ...
    setTransportistas(prev=>[...prev,{...formE,id:Date.now()}]);
    setModEmpresa(false);setFormE({nombre:"",nit:"",contacto:"",tel:""});
    showToast("✓ Empresa registrada","success");
  };

  const inscribirConductor=()=>{
    if(!formC.nombre.trim()||!formC.placa.trim()){showToast("Nombre y placa son obligatorios","error");return;}
    if(!formC.user.trim()||!formC.pass.trim()){showToast("Usuario y contraseña son obligatorios","error");return;}
    if(usuarios.find(u=>u.user===formC.user.trim())){showToast("Ese usuario ya existe","error");return;}
    const newId=Date.now();
    const emp=modCond;
    setConductores(prev=>[...prev,{id:newId,nombre:formC.nombre.trim(),placa:formC.placa.trim(),celular:formC.celular.trim(),nit_proveedor:emp.nit,empresa:emp.nombre,activo:true}]);
    setUsuarios(prev=>[...prev,{id:newId,nombre:formC.nombre.trim(),user:formC.user.trim(),pass:formC.pass.trim(),rol:"conductor",placa:formC.placa.trim(),celular:formC.celular.trim(),nit_proveedor:emp.nit,empresa:emp.nombre}]);
    setModCond(null);setFormC({nombre:"",placa:"",celular:"",user:"",pass:""});
    showToast(`✓ Conductor inscrito en ${emp.nombre}`,"success");
  };

  return (
    <div>
      {/* Banner empresa propia */}
      {esMia&&(
        <Card style={{background:`linear-gradient(135deg,${P[950]},${P[700]})`,marginBottom:22}}>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            ...
            <div>
              ...
              <p style={{margin:"4px 0 0",color:P[300],fontSize:13}}>NIT: {miNit} · {misConductores.filter(c=>c.activo).length} conductor(es)</p>
            </div>
          </div>
        </Card>
      )}
            ...
            return (
              <Card key={t.id} style={{borderLeft:`4px solid ${P[500]}`}}>
                ...
                <div style={{marginTop:12}}>
                  <Btn size="sm" variant="secondary" onClick={()=>{setModCond(t);setFormC({nombre:"",placa:"",celular:"",user:"",pass:""});}}>+ Inscribir Conductor</Btn>
                </div>
              </Card>
            );
          })}
          {misEmpresas.length===0&&<p style={{color:"#94a3b8",fontSize:13}}>Sin empresas registradas. Agrega una con el botón.</p>}
        </div>
      )}

      {/* Conductores de la empresa/todas */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <h3 style={{margin:0,color:P[800],fontWeight:800}}>🚗 {esMia?"Mis Conductores":"Todos los Conductores"}</h3>
        {esMia&&<Btn onClick={()=>{setModCond({nit:miNit,nombre:user.empresa||user.nombre});setFormC({nombre:"",placa:"",celular:"",user:"",pass:""});}}>+ Inscribir Conductor</Btn>}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:14}}>
        {misConductores.map(c=>(
          <Card key={c.id} style={{borderLeft:`3px solid ${P[400]}`}}>
            ...
            {c.celular&&<div style={{fontSize:12,color:"#64748b",marginTop:2}}>📱 {c.celular}</div>}
        ...
        {misConductores.length===0&&<p style={{color:"#94a3b8",fontSize:13}}>Sin conductores inscritos.</p>}
      </div>
        ...
        <Modal title="Nueva Empresa Transportista" onClose={()=>setModEmpresa(false)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              ...
              <Btn onClick={crearEmpresa}>💾 Guardar Empresa</Btn>
            </div>
          </div>
        </Modal>
      )}
        ...
        <Modal title={`Inscribir Conductor — ${modCond.nombre}`} onClose={()=>setModCond(null)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{borderTop:`1px solid ${P[100]}`,paddingTop:12}}>
              <p style={{fontSize:12,fontWeight:700,color:P[700],margin:"0 0 10px",textTransform:"uppercase"}}>Acceso al Sistema</p>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                <Field label="Usuario (login)" value={formC.user} onChange={fc("user")} required placeholder="juan.perez"/>
                <Field label="Contraseña" value={formC.pass} onChange={fc("pass")} required type="password" placeholder="••••••••"/>
              </div>
            </div>
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
...
function ResumenTransportador({ pedidos, conductores }) {
  ...
  const misPeds=selCond?pedidos.filter(p=>p.conductor_id===parseInt(selCond)):[];
  ...
  return (
    <div>
      <h2 style={{margin:"0 0 22px",color:P[800],fontWeight:900}}>📋 Resumen por Transportador</h2>
          ...
          <Card style={{background:`linear-gradient(135deg,${P[800]},${P[600]})`,marginBottom:20}}>
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              ...
              <div style={{marginLeft:"auto",textAlign:"right"}}>
                <div style={{color:"#fff",fontSize:32,fontWeight:900,lineHeight:1}}>{misPeds.length}</div>
                <div style={{color:P[300],fontSize:12}}>pedidos</div>
                <div style={{color:"#fff",fontSize:24,fontWeight:900,lineHeight:1,marginTop:4}}>{totalCajas}</div>
                <div style={{color:P[300],fontSize:12}}>cajas</div>
              </div>
            </div>
          </Card>
          <Card style={{padding:0,overflow:"hidden"}}>
...
function Usuarios({ usuarios, setUsuarios, showToast }) {
  ...
  const [form,setForm]=useState({nombre:"",user:"",pass:"",rol:"operador",nit:"",empresa:"",placa:"",celular:"",nit_proveedor:""});
  const f=k=>v=>setForm(p=>({...p,[k]:v}));
  const roleColors={admin:P[600],operador:P[400],transportista:"#0891b2",conductor:"#059669",cliente:"#d97706"};

  const abrirEditar=(u)=>{
    setModEditar(u);
    setForm({nombre:u.nombre,user:u.user,pass:u.pass,rol:u.rol,nit:u.nit||"",empresa:u.empresa||"",placa:u.placa||"",celular:u.celular||"",nit_proveedor:u.nit_proveedor||""});
  };
  ...
  const guardarNuevo=()=>{
    ...
    setModNuevo(false);
    setForm({nombre:"",user:"",pass:"",rol:"operador",nit:"",empresa:"",placa:"",celular:"",nit_proveedor:""});
    showToast("✓ Usuario creado","success");
  };

  const guardarEdicion=()=>{
    if(!form.nombre.trim()||!form.user.trim()||!form.pass.trim()){showToast("Campos obligatorios vacíos","error");return;}
    const conflict=usuarios.find(u=>u.user===form.user.trim()&&u.id!==modEditar.id);
    if(conflict){showToast("Ese usuario ya lo usa otra persona","error");return;}
    setUsuarios(prev=>prev.map(u=>u.id===modEditar.id?{...u,...form}:u));
    setModEditar(null);
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
        ...
        <Btn onClick={()=>setModNuevo(true)}>+ Nuevo Usuario</Btn>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:14}}>
        {usuarios.map(u=>(
          <Card key={u.id} style={{borderTop:`3px solid ${roleColors[u.rol]||P[400]}`}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:12}}>
              ...
              {u.id!==1&&<Btn size="sm" variant="danger" onClick={()=>eliminar(u.id)}>× Eliminar</Btn>}
            </div>
          </Card>
        ))}
      </div>
      ...
      {/* Modal nuevo */}
      {modNuevo&&(
        <Modal title="Nuevo Usuario" onClose={()=>setModNuevo(false)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              <Btn variant="secondary" onClick={()=>setModNuevo(false)}>Cancelar</Btn>
              <Btn onClick={guardarNuevo}>💾 Crear Usuario</Btn>
            </div>
          </div>
        </Modal>
      )}
      ...
      {/* Modal editar */}
      {modEditar&&(
        <Modal title={`Editar Usuario — ${modEditar.nombre}`} onClose={()=>setModEditar(null)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <Field label="Nombre completo" value={form.nombre} onChange={f("nombre")} required/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              <Field label="Usuario (login)" value={form.user} onChange={f("user")} required/>
              <Field label="Contraseña" value={form.pass} onChange={f("pass")} required type="password"/>
            </div>
            <Field label="Rol" value={form.rol} onChange={f("rol")} as="select"
              options={Object.entries(ROLES).map(([k,v])=>({value:k,label:v}))}/>
            <CamposExtra/>
...
function MisPedidosConductor({ pedidos, setPedidos, user, conductores, ciudades, showToast }) {
  const [modDet,setModDet]=useState(null);
  // El conductor se identifica por user.id que coincide con conductor.id
  ...
  return (
    <div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          ...
          <div>
            ...
            <p style={{margin:"3px 0 0",color:P[300],fontSize:13}}>
              Placa: {user.placa||"—"}{user.celular&&` · 📱 ${user.celular}`} · {misPeds.filter(p=>["pendiente","en_transito"].includes(p.estado)).length} activo(s) de {misPeds.length} total
            </p>
          </div>
        </div>
      </Card>
        ...
        {misPeds.map(p=>(
          <Card key={p.id} style={{cursor:"pointer",borderLeft:`4px solid ${ESTADOS_PEDIDO[p.estado]?.color||"#ccc"}`}} onClick={()=>setModDet(p)}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
              <div>
                <div style={{fontWeight:900,color:P[700],fontSize:16}}>{p.guia_interna||p.id}</div>
      ...
      {modDet&&<ModalDetalle pedido={modDet} conductores={conductores} ciudades={ciudades} onClose={()=>setModDet(null)} setPedidos={setPedidos} showToast={showToast} canEdit={false}/>}
    </div>
  );
}
...
function MiUbicacion() {
  ...
  const iniciar=()=>{setErr("");if(!navigator.geolocation){setErr("Tu dispositivo no soporta GPS.");return;}
    wRef.current=navigator.geolocation.watchPosition(pos=>{setLat(pos.coords.latitude);setLng(pos.coords.longitude);setOn(true);},e=>{setErr(`Error GPS: ${e.message}`);setOn(false);},{enableHighAccuracy:true,maximumAge:15000,timeout:20000});
  };
...
// ─── Consultas (cliente) ──────────────────────────────────────────────────────
function Consultas({ pedidos, conductores, ciudades, showToast }) {
  const [busq,setBusq]=useState("");const [modDet,setModDet]=useState(null);
  ...
  return (
    <div>
      ...
      <Card style={{padding:14,marginBottom:16}}>
        <input value={busq} onChange={e=>setBusq(e.target.value)} placeholder="🔍 Buscar por guía, N° pedido, factura, cliente o ciudad..." style={iSt}/>
      </Card>
            ...
            <Card key={p.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12,padding:18}}>
              <div style={{flex:1,minWidth:200}}>
                <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:6,flexWrap:"wrap"}}>
                  {p.guia_interna&&<span style={{fontWeight:900,color:P[600],fontSize:13,fontFamily:"monospace"}}>{p.guia_interna}</span>}
                  <span style={{fontWeight:800,color:P[700],fontSize:15}}>{p.id}</span>
                  <Badge estado={p.estado}/>
                ...
                <div style={{fontSize:12,color:"#64748b",marginTop:3}}>📦 Factura: {p.factura} · {p.cajas} cajas · 🏙️ {p.ciudad_nombre} · 📍 {p.direccion}</div>
                <div style={{fontSize:12,color:"#94a3b8",marginTop:2}}>
                  {p.tipo==="paqueteria"?`Guía: ${p.guia_paqueteria}`:cond?`🚗 ${cond.nombre} · ${p.placa}`:"Sin conductor"}
                  {" · "}📅 {p.fecha_estimada||"—"}
...
function Login({ onLogin, usuarios }) {
  ...
  const login=(e)=>{e.preventDefault();setErr("");const found=usuarios.find(x=>x.user===u.trim()&&x.pass===p);if(found)onLogin(found);else setErr("Usuario o contraseña incorrectos.");};
  const demos=[{l:"👑 Admin",u:"admin",p:"1039456779"},{l:"⚙️ Operador",u:"operador",p:"op123"},{l:"🏢 Transportista",u:"veloz",p:"trans123"},{l:"🚗 Conductor",u:"driver1",p:"cond123"},{l:"📦 Cliente",u:"cliente",p:"cli123"}];
  return (
...
function SidebarApp({ user, activeTab, setActiveTab, onLogout, collapsed, setCollapsed }) {
  const menus = {
    admin:        [["dashboard","📊","Dashboard"],["pedidos","📦","Pedidos"],["rastreo","🗺️","Rastreo GPS"],["conductores","🚗","Conductores"],["transportistas","🏢","Transportistas"],["resumen","📋","Resumen Transportador"],["ciudades","🏙️","Ciudades / DANE"],["paqueterias","📦","Paqueterías"],["usuarios","👥","Usuarios"]],
    operador:     [["dashboard","📊","Dashboard"],["pedidos","📦","Pedidos"],["rastreo","🗺️","Rastreo GPS"],["conductores","🚗","Conductores"],["resumen","📋","Resumen Transportador"]],
...
export default function SomosProTracking() {
  ...
  const p={pedidos,setPedidos,conductores,setConductores,transportistas,setTransportistas,usuarios,setUsuarios,ciudades,setCiudades,paqueterias,setPaqueterias,showToast,user};
  ...
  const renderContent=()=>{
    switch(tab){
      ...
      case "conductores":    return <Conductores conductores={conductores} setConductores={setConductores} pedidos={pedidos} showToast={showToast} usuarios={usuarios} setUsuarios={setUsuarios}/>;
      ...
      case "resumen":        return <ResumenTransportador pedidos={pedidos} conductores={conductores}/>;
      ...
      case "consultas":      return <Consultas pedidos={pedidos} conductores={conductores} ciudades={ciudades} showToast={showToast}/>;
