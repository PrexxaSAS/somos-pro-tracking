...
function ModalCSVGuias({ onClose, pedidos, showToast, recargar }) {
  ...
  const aplicar = async () => {
    ...
    for (const m of paraActualizar) {
      ...
      if (m.destino) {
        ...
        const ciudad = (window._ciudades||[]).find(c => c.code === m.destino);
...
function Pedidos({ pedidos, setPedidos, conductores, ciudades, showToast, paqueterias, transportistas, recargar }) {
  ...
  return (
    <div>
      ...
      {modGuias&&<ModalCSVGuias onClose={()=>setModGuias(false)} pedidos={pedidos} showToast={showToast} recargar={recargar}/>}
    </div>
  );
}
