...
function ModalDetalle({ pedido, conductores, ciudades, onClose, setPedidos, showToast, canEdit }) {
  ...
  const guardar = async () => {
    ...
    if (window._sb) {
      const { error } = await window._sb.from("pedidos").update(cambios).eq("id", pedido.id);
  ...
  const subirFotos = async (fotos) => {
    ...
    setPedidos(prev=>prev.map(p=>p.id===pedido.id?{...p,...cambios}:p));
    if (window._sb) {
      await window._sb.from('pedidos').update(cambios).eq('id', pedido.id);
      if (window._recargar) await window._recargar();
    }
    setVerCamara(false);
...
function Pedidos({ pedidos, setPedidos, conductores, ciudades, showToast }) {
  ...
  const guardar = async () => {
    ...
    if (sb) {
      const { error } = await sb.from("pedidos").insert(nuevo);
  ...
  return (
    <div>
      ...
      {modCSV&&<ModalCSVPedidos onClose={()=>setModCSV(false)} ciudades={ciudades} onImportar={async (rows)=>{
  const conGuias=rows.map((r,i)=>({...r,guia_interna:r.tipo!=="paqueteria"?generarGuia([...pedidos,...rows.slice(0,i)]):null,estado_despacho:r.estado_despacho||"despachado",novedad:false,soportes_data:[]}));
  if(sb){
    for(const p of conGuias){
      const {error}=await sb.from("pedidos").insert(p);
      if(error){showToast("Error importando "+p.id+": "+error.message,"error");return;}
    }
    await recargar();
  } else {
    setPedidos(prev=>[...conGuias,...prev]);
  }
  setModCSV(false);
...
function Conductores({ conductores, pedidos, showToast, transportistas, sb, recargar }) {
  ...
  const guardar = async () => {
    ...
    try {
      ...
      const { data: cData, error: cErr } = await sb.from('conductores').insert({
        nombre: form.nombre.trim(),
        cedula: form.cedula.trim(),
        placa: form.placa.trim(),
        celular: form.celular.trim(),
      ...
      await sb.from('usuarios').update({ conductor_id: cData.id }).eq('id', uData.id);
...
function Transportistas({ transportistas, conductores, showToast, user, sb, recargar }) {
  ...
  const crearEmpresa = async () => {
    ...
    try {
      ...
      const { error: tErr } = await sb.from('transportistas').insert({
  ...
  const guardarEdicionEmpresa = async () => {
    ...
    setGuardando(true);
    await sb.from('transportistas').update({nombre:formE.nombre.trim(),contacto:formE.contacto.trim(),tel:formE.tel.trim()}).eq('id',modEditEmp.id);
    if (formE.pass_login.trim() && modEditEmp.usuario_id) {
      await sb.from('usuarios').update({nombre:formE.nombre.trim(),pass:formE.pass_login.trim()}).eq('id',modEditEmp.usuario_id);
    }
  ...
  const inscribirConductor = async () => {
    ...
    try {
      ...
      if (uErr) { showToast("Error usuario: "+uErr.message,"error"); setGuardando(false); return; }
      const { error: cErr } = await sb.from('conductores').insert({
        nombre:formC.nombre.trim(), cedula:formC.cedula.trim(), placa:formC.placa.trim(),
        celular:formC.celular.trim(), nit_proveedor:emp.nit, empresa:emp.nombre,
        activo:true, usuario_id:uData.id,
      });
  ...
  const guardarEdicionConductor = async () => {
    ...
    if (modEdit.usuario_id) {
      await sb.from('usuarios').update({nombre:formEdit.nombre.trim(),placa:formEdit.placa.trim(),celular:formEdit.celular.trim()}).eq('id',modEdit.usuario_id);
    }
...
function Ciudades({ ciudades, showToast, sb, recargar }) {
  ...
  const guardar=async()=>{
    ...
    if(sb){ await sb.from('ciudades').upsert({code:form.code.trim(),name:form.name.trim()},{onConflict:'code'}); if(recargar)await recargar(); }
...
function GestionPaqueterias({ paqueterias, showToast, sb, recargar }) {
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
        {paqueterias.map((p,i)=>(
          <Card key={i} style={{padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <button onClick={async ()=>{if(sb){await sb.from('paqueterias').delete().eq('nombre',p);if(recargar)await recargar();} showToast("Eliminado","info");}}
...
function Consultas({ pedidos, conductores, ciudades, devoluciones = [], recogidas = [], showToast }) {
  const [busq,    setBusq]    = useState("");
  const [modMapa, setModMapa] = useState(null); // pedido seleccionado para ver mapa
            ...
            <Card key={p.id} style={{ padding:18, borderLeft:`4px solid ${ESTADOS_PEDIDO[p.estado]?.color||P[300]}` }}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10,flexWrap:"wrap",gap:8}}>
                <div>
                  ...
                  <div style={{fontWeight:800,color:P[800],fontSize:14}}>{p.id} {p.factura&&<span style={{fontWeight:400,color:"#64748b",fontSize:12}}>· Factura: {p.factura}</span>}</div>
                </div>
              ...
              <div style={{display:"flex",gap:8,marginTop:12,flexWrap:"wrap"}}>
                {/* Soportes — sí puede descargar */}
                {(p.soportes_data||[]).length>0?(
                  <Btn size="sm" variant="success" onClick={()=>generarPDFSoportes(p,[])}>📄 Ver PDF Soportes ({p.soportes_data.length})</Btn>
                ):(p.soportes||[]).length>0?(
                  <Btn size="sm" variant="success" onClick={()=>showToast(`📎 ${p.soportes.length} soporte(s) registrado(s)`, "info")}>📎 Soportes ({p.soportes.length})</Btn>
                ):null}
                {/* Rastreo satelital */}
                <Btn size="sm" variant="secondary" onClick={()=>setModMapa(modMapa?.id===p.id?null:p)}>
                  {modMapa?.id===p.id?"🗺️ Ocultar Mapa":"🗺️ Rastreo Satelital"}
                </Btn>
              </div>
              {/* Mapa inline satelital */}
              {modMapa?.id===p.id&&(
                <div style={{marginTop:14,borderRadius:12,overflow:"hidden",border:`2px solid ${P[200]}`}}>
                  <iframe title={"mapa-"+p.id} width="100%" height="300" style={{border:"none",display:"block"}}
                    src={`https://maps.google.com/maps?q=${encodeURIComponent((p.direccion||"")+(ciudad?", "+ciudad.name:"")+", Colombia")}&output=embed&z=15`}
                    allowFullScreen loading="lazy"/>
                  <div style={{background:P[50],padding:"8px 14px",fontSize:12,color:P[700]}}>
                    📍 Destino: {p.direccion}, {ciudad?.name} · Código DANE: {p.ciudad_codigo}
                    {cond&&<span style={{marginLeft:12}}>🚗 {cond.nombre} · {p.placa}{cond.celular&&` · 📱 ${cond.celular}`}</span>}
                  </div>
                </div>
              )}
            </Card>
...
//  MÓDULO DEVOLUCIONES
// ══════════════════════════════════════════════════════════
function ModuloDevoluciones({ devoluciones, setDevoluciones, conductores, ciudades, showToast, user, sb, recargar }) {
  const [modNueva, setModNueva] = useState(false);
  const [modDet,   setModDet]   = useState(null);
  const [busq,     setBusq]     = useState("");
  const fileRef = useRef(null);
  ...
  const crear = async () => {
    ...
    if (sb) {
      const { error } = await sb.from('devoluciones').insert(nueva);
      if (error) { showToast("Error: "+error.message,"error"); return; }
      await recargar();
    } else { setDevoluciones(prev=>[nueva,...prev]); }
  ...
  const asignar = async (id, condId, novedad) => {
    ...
    setDevoluciones(prev=>prev.map(d=>d.id===id?{...d,...cambios}:d));
    if (sb) { await sb.from('devoluciones').update(cambios).eq('id',id); await recargar(); }
  };
...
//  MÓDULO RECOGIDAS
// ══════════════════════════════════════════════════════════
function ModuloRecogidas({ recogidas, setRecogidas, conductores, ciudades, showToast, user, sb, recargar }) {
  const [modNueva, setModNueva] = useState(false);
  const [modDet,   setModDet]   = useState(null);
  const [busq,     setBusq]     = useState("");
  const fileRef = useRef(null);
  ...
  const crear = async () => {
    ...
    if (sb) {
      const { error } = await sb.from('recogidas').insert(nueva);
      if (error) { showToast("Error: "+error.message,"error"); return; }
      await recargar();
    } else { setRecogidas(prev=>[nueva,...prev]); }
    setModNueva(false); setForm(vacio);
  ...
  const asignar = async (id, condId, novedad) => {
    ...
    const cambios = { conductor_id:cond?cond.id:null, placa:cond?cond.placa:null,
      ...
      estado:cond?"en_transito":"sin_asignar",
      novedad:novedad!==undefined?novedad:false };
    setRecogidas(prev=>prev.map(r=>r.id===id?{...r,...cambios}:r));
    if (sb) { await sb.from('recogidas').update(cambios).eq('id',id); await recargar(); }
  };
...
function ModuloPQRS({ pqrs, setPqrs, pedidos, showToast, user, sb, recargar }) {
  const MOTIVOS = [
  ...
  const crear = async () => {
    ...
    if (sb) {
      const { error } = await sb.from('pqrs').insert(nueva);
      if (error) { showToast("Error: "+error.message,"error"); return; }
      await recargar();
    } else { setPqrs(prev=>[nueva,...prev]); }
    setModNueva(false); setForm(vacio);
  ...
  const guardarGestion = async () => {
    ...
    setPqrs(prev=>prev.map(p=>p.id===modGestion.id?{...p,...cambios}:p));
    if (sb) { await sb.from('pqrs').update(cambios).eq('id', modGestion.id); await recargar(); }
  ...
  const cerrar = async (id, estado) => {
    ...
    if (sb) { await sb.from('pqrs').update({estado}).eq('id', id); await recargar(); }
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:12,marginBottom:20}}>
        {Object.entries(ESTADOS_PQRS).map(([k,v])=>(
          <Card key={k} style={{textAlign:"center",padding:14,borderTop:`3px solid ${v.color}`,cursor:"pointer",background:filtroEst===k?v.bg:"#fff"}} onClick={()=>setFiltroEst(filtroEst===k?"todos":k)}>
      ...
      <Card style={{padding:14,marginBottom:16}}>
        ...
        <div style={{marginTop:8,fontSize:12,color:"#64748b"}}>{filt.length} PQRS encontrada(s)</div>
      </Card>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
                  ...
                  {p.respuesta&&(
                    <div style={{background:"#f0fdf4",borderRadius:8,padding:"8px 12px",border:"1px solid #86efac"}}>
                      <div style={{fontSize:12,fontWeight:700,color:"#059669",marginBottom:4}}>
                        ✅ Gestión — {p.gestionado_por} · {p.fecha_gestion}
                      </div>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8,minWidth:130}}>
                  {esOperador&&p.estado!=="cerrada"&&p.estado!=="rechazada"&&(
                    <Btn size="sm" variant="primary" onClick={()=>{setModGestion(p);setGestion(p.respuesta||"");}}>
                      ✏️ Gestionar
                    </Btn>
                  )}
        ...
        <Modal title="Nueva PQRS" onClose={()=>{setModNueva(false);setForm(vacio);}}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{background:"#fffbeb",borderRadius:10,padding:10,fontSize:12,color:"#92400e",fontWeight:600}}>
              Se generará automáticamente un número de caso (PQRS-{new Date().getFullYear()}-XXXX). Todos los campos son obligatorios.
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              <Field label="N° Factura *"      value={form.factura}    onChange={f("factura")}    placeholder="FAC-2200"/>
              <Field label="N° Pedido Ref. *"  value={form.pedido_ref} onChange={f("pedido_ref")} placeholder="PED-001"/>
            </div>
            <Field label="Motivo de la PQRS *" value={form.motivo} onChange={f("motivo")} as="select"
            ...
            <Field label="Descripción detallada *" value={form.descripcion} onChange={f("descripcion")} as="textarea"
              placeholder="Describe con detalle la situación, fecha del evento, personas involucradas, consecuencias..."/>
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              <Btn variant="secondary" onClick={()=>{setModNueva(false);setForm(vacio);}}>Cancelar</Btn>
        ...
        <Modal title={`Gestionar PQRS — ${modGestion.id}`} onClose={()=>setModGestion(null)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <Field label="Respuesta / Gestión realizada *" value={gestion} onChange={setGestion} as="textarea"
              placeholder="Describe las acciones tomadas, compensaciones, compromisos, fechas de solución..."/>
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
...
export default function SomosProTracking() {
  ...
  const renderContent = () => {
    ...
    switch (tab) {
      ...
      case "pedidos":        return <Pedidos pedidos={pedidos} conductores={conductores} ciudades={ciudades} showToast={showToast} paqueterias={paqueterias} sb={sb} recargar={re}/>;
      case "rastreo":        return <RastreoGPS pedidos={pedidos} conductores={conductores} ciudades={ciudades}/>;
      case "conductores":    return <Conductores conductores={conductores} pedidos={pedidos} showToast={showToast} transportistas={transportistas} sb={sb} recargar={re}/>;
      case "transportistas": return <Transportistas transportistas={transportistas} conductores={conductores} showToast={showToast} user={{rol:"admin",nombre:"Admin"}} sb={sb} recargar={re}/>;
      ...
      case "ciudades":       return <Ciudades ciudades={ciudades} showToast={showToast} sb={sb} recargar={re}/>;
      case "paqueterias":    return <GestionPaqueterias paqueterias={paqueterias} showToast={showToast} sb={sb} recargar={re}/>;
      case "usuarios":       return <Usuarios usuarios={usuarios} showToast={showToast} sb={sb} recargar={re}/>;
      case "mi_empresa":     return <Transportistas transportistas={transportistas} conductores={conductores} showToast={showToast} user={user} sb={sb} recargar={re}/>;
      case "mis_pedidos":    return <MisPedidosConductor pedidos={pedidos} user={user} conductores={conductores} ciudades={ciudades} showToast={showToast} sb={sb} recargar={re}/>;
      ...
      case "pqrs":           return <ModuloPQRS pqrs={pqrs} pedidos={pedidos} showToast={showToast} user={user} sb={sb} recargar={re}/>;
      case "devoluciones":   return <ModuloDevoluciones devoluciones={devoluciones} conductores={conductores} ciudades={ciudades} showToast={showToast} user={user} sb={sb} recargar={re}/>;
      case "recogidas":      return <ModuloRecogidas recogidas={recogidas} conductores={conductores} ciudades={ciudades} showToast={showToast} user={user} sb={sb} recargar={re}/>;
...
function Usuarios({ usuarios, showToast, sb, recargar }) {
  ...
  const crearUsuario = async () => {
    ...
    setGuardando(true);
    const { error } = await sb.from('usuarios').insert({...form});
  ...
  const guardarEdicion = async () => {
    ...
    const { error } = await sb.from('usuarios').update({...form,pass:passNueva}).eq('id',modEditar.id);
    if (error) { showToast("Error: "+error.message,"error"); setGuardando(false); return; }
    setModEditar(null);
  ...
  const eliminar = async (uid, uname) => {
    ...
    await sb.from('usuarios').delete().eq('id',uid);
