...
import { USUARIOS_INICIALES, CONDUCTORES_INICIALES, TRANSPORTISTAS_INICIALES, PEDIDOS_INICIALES } from './DataStore';
            ...
            <table style={{ width: "100%", fontSize: 14, borderCollapse: "collapse" }}>
              <tbody>
                {[["Cliente:", pedido.cliente], ["Dirección:", pedido.direccion], ["Ciudad:", ciudad?.name || "—"], ["Cód. DANE:", pedido.ciudad_codigo], ["Factura:", pedido.factura], ["Cajas:", pedido.cajas]].map(([k, v]) => (
                  <tr key={k}><td style={{ fontWeight: 700, color: "#475569", paddingBottom: 5, paddingRight: 8, whiteSpace: "nowrap" }}>{k}</td><td style={{ color: "#1e293b" }}>{v}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <div style={{ background: P[50], borderRadius: 12, padding: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(190px,1fr))", gap: 8, fontSize: 13, color: "#64748b" }}>
            ...
            <span>📦 Cajas: <strong>{pedido.cajas}</strong></span>
            <span>📅 Estimado: {pedido.fecha_estimada || "—"}</span>
            <span>✅ Entrega real: {pedido.fecha_real || "Pendiente"}</span>
          </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 14 }}>
        ...
        {lista.length === 0 && <p style={{ color: "#94a3b8", fontSize: 13 }}>Sin conductores inscritos aún.</p>}
      </div>
      {modal && (
