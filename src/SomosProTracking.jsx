...
function ModalDetalle({ pedido, conductores, ciudades, onClose, setPedidos, showToast, canEdit }) {
  ...
  const guardar = async () => {
    ...
    if (supabase) {
      const { error } = await window._supabase.from("pedidos").update(cambios).eq("id", pedido.id);
  ...
  const subirFotos = async (fotos) => {
    ...
    setPedidos(prev=>prev.map(p=>p.id===pedido.id?{...p,...cambios}:p));
    if (supabase) {
      await window._supabase.from('pedidos').update(cambios).eq('id', pedido.id);
      await cargarTodo();
    }
    setVerCamara(false);
...
function Ciudades({ ciudades, showToast, sb, recargar }) {
  ...
  const guardar=async()=>{
    ...
    if(supabase){ await supabase.from('ciudades').upsert({code:form.code.trim(),name:form.name.trim()},{onConflict:'code'}); if(recargar)await cargarTodo(); }
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
        {paqueterias.map((p,i)=>(
          <Card key={i} style={{padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <button onClick={async ()=>{if(sb){await supabase.from('paqueterias').delete().eq('nombre',p);if(recargar)await cargarTodo();} showToast("Eliminado","info");}}
...
function ModuloDevoluciones({ devoluciones, setDevoluciones, conductores, ciudades, showToast, user, sb, recargar }) {
  ...
  const crear = async () => {
    ...
    if (supabase) {
      const { error } = await supabase.from('devoluciones').insert(nueva);
  ...
  const asignar = async (id, condId, novedad) => {
    ...
    if (supabase) { await supabase.from('devoluciones').update(cambios).eq('id',id); await cargarTodo(); }
  };
...
function ModuloRecogidas({ recogidas, setRecogidas, conductores, ciudades, showToast, user, sb, recargar }) {
  ...
  const crear = async () => {
    ...
    if (supabase) {
      const { error } = await supabase.from('recogidas').insert(nueva);
      if (error) { showToast("Error: "+error.message,"error"); return; }
      await cargarTodo();
  ...
  const asignar = async (id, condId, novedad) => {
    ...
    if (supabase) { await supabase.from('recogidas').update(cambios).eq('id',id); await cargarTodo(); }
  };
...
function ModuloPQRS({ pqrs, setPqrs, pedidos, showToast, user, sb, recargar }) {
  ...
  const guardarGestion = async () => {
    ...
    if (supabase) { await supabase.from('pqrs').update(cambios).eq('id', modGestion.id); await cargarTodo(); }
  ...
  const cerrar = async (id, estado) => {
    ...
    if (supabase) { await supabase.from('pqrs').update({estado}).eq('id', id); await cargarTodo(); }
