  ...
  return (
    <div>
      ...
      <Card style={{padding:14,marginBottom:16}}>
        ...
        {filtradas.length>0&&(fechaDesde||fechaHasta||busq)&&(
          <div style={{marginTop:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontSize:12,color:"#64748b"}}>
              {filtradas.length} factura(s) en el filtro
              {fechaDesde&&fechaHasta?` · ${fechaDesde} al ${fechaHasta}`:fechaDesde?` · desde ${fechaDesde}`:fechaHasta?` · hasta ${fechaHasta}`:""}
            </span>
            <Btn size="sm" variant="success" onClick={()=>{
              ...
              showToast(`✓ CSV descargado · ${lineasTodas.length} líneas`,"success");
            }}>
              ⬇ Descargar Informe ({filtradas.length} facturas)
            </Btn>
          </div>
        )}
      </Card>
...
function ModalFacturaGuias({ factura, pedidos, transportistas, showToast, recargar, onClose, formatCOP }) {
  ...
  const pedidosFiltrados = pedidos.filter(p => {
    const yaRelacionado = pedidosRel.includes(p.id);
    if (yaRelacionado) return false;
    if (busqPed) {
        ...
        <div style={{background:"#f8fafc",borderRadius:12,padding:14,border:"1px solid #e2e8f0"}}>
          ...
          <div style={{display:"flex",gap:10,alignItems:"flex-end"}}>
            <Field label="" value={pedidoSel} onChange={setPedidoSel} as="select"
              style={{flex:1}}
              options={[
                {value:"",label:"— Seleccione pedido —"},
                ...pedidosFiltrados.slice(0,50).map(p=>({
                  value:p.id,
                  label:`${p.id} · ${p.guia_interna||"sin guía"} · ${p.cliente} · ${p.cajas} cajas`
                }))
              ]}/>
...
export default function SomosProTracking() {
  ...
  const cargarTodo = async () => {
    ...
    try {
      ...
      setPqrs(pqrsd || []);
      setPromesas(prom || []);
      // Cargar facturas por separado para no bloquear la carga principal
      try {
        const { data: factData } = await supabase
          .from('facturas_proveedor')
          .select('*, factura_guias(*, pedidos(*))')
